...
type PizzaWithStock = PizzaRow & { stock: number; image_path?: string | null; image_url?: string | null };
...
export default function AdminPizzas() {
  ...
  const checkHasImagePathCol = async () => {
    try {
      ...
      const { error } = await supabase.from('pizzas').select('image_path').limit(1);
      if (error) {
  ...
  const uploadImageToStorage = async (pizzaId: string, file: File) => {
    ...
    const { error: uploadError } = await supabase.storage.from('pizzas').upload(filePath, file, {
      cacheControl: '3600',
      upsert: false,
      contentType: file.type,
    });
    if (uploadError) throw uploadError;
    const { data: urlData } = supabase.storage.from('pizzas').getPublicUrl(filePath);
  ...
  const fetchPizzas = async () => {
    ...
    try {
      ...
      const selectCols = hasImagePathCol ? 'id,name,description,price,image_path,stock,created_at' : 'id,name,description,price,image_url,stock,created_at';
      const { data, error } = await supabase
        .from('pizzas')
        .select(selectCols)
        .order('created_at', { ascending: false });
      ...
      const mapped = await Promise.all(
        rows.map(async (r: any) => {
          ...
          if (hasImagePathCol && r.image_path) {
            const { data: urlData } = supabase.storage.from('pizzas').getPublicUrl(r.image_path);
  ...
  const handleSave = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (editing) {
        ...
        const { data, error } = await supabase.from('pizzas').update(updatePayload).eq('id', editing.id).select().single();
        if (error) throw error;
        const updated = data as unknown as PizzaWithStock;
        ...
        if (hasImagePathCol && updated.image_path) {
          const { data: urlData } = supabase.storage.from('pizzas').getPublicUrl(updated.image_path);
          publicUrl = urlData?.publicUrl ?? publicUrl;
        }

        const mapping: PizzaWithStock = {
          ...updated,
          price: typeof updated.price === 'number' ? updated.price : parseFloat(updated.price ?? 0),
          stock: typeof updated.stock === 'number' ? updated.stock : parseInt(updated.stock ?? '0', 10),
          image_url: publicUrl ?? null,
        };

        setPizzas((prev) => prev.map((p) => (p.id === mapping.id ? mapping : p)));
        setEditing(null);
      } else {
        ...
        const { data: createdData, error: insertError } = await supabase.from('pizzas').insert(insertPayload).select().single();
        ...
        if (imageFile) {
          ...
          if (hasImagePathCol) {
            const { data: updatedData, error: updateError } = await supabase
              .from('pizzas')
              .update({ image_path: uploadRes.path, image_url: uploadRes.publicUrl })
              .eq('id', created.id)
              .select()
              .single();
        ...
        const mapped: PizzaWithStock = {
          ...created,
          price: typeof created.price === 'number' ? created.price : parseFloat(created.price ?? 0),
          stock: typeof created.stock === 'number' ? created.stock : parseInt(created.stock ?? '0', 10),
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8 rounded-lg border border-border bg-surface p-6">
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <p className="text-xs text-text-secondary mt-1">
                También puedes subir un archivo en el selector de abajo. Si subes un archivo, ese tendrá prioridad.
              </p>
            </div>
          </div>
          ...
          <div>
            <label className="mb-1 block text-sm font-medium">Subir imagen (archivo)</label>
            <input
              type="file"
              accept="image/*"
            ...
            {imagePreview && (
              <div className="mt-2">
                ...
                <img src={imagePreview} alt="preview" className="mt-1 h-32 w-32 rounded-md object-cover" />
              </div>
            )}
          </div>

          <div className="flex items-center gap-2">
            <Button type="submit" disabled={uploading}>{uploading ? 'Guardando...' : (editing ? 'Actualizar Pizza' : 'Crear Pizza')}</Button>
            {editing && <Button variant="outline" onClick={prepareCreate}>Cancelar</Button>}
            {error && <p className="text-sm text-red-500">{error}</p>}
          </div>
        </form>
        <div className="mt-3 text-xs text-text-secondary">
          <strong>Nota:</strong> las imágenes se guardan en el bucket público <code>pizzas</code>. Si tu tabla tiene la columna <code>image_path</code>, se guardará la ruta en storage (recomendado). Si no existe esa columna, el sistema guarda la URL pública en <code>image_url</code> para mantener compatibilidad.
        </div>
      </div>
